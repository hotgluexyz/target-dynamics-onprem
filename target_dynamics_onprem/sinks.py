...
class PurchaseInvoice(DynamicOnpremSink):
    ...
    def upsert_record(self, record: dict, context: dict):
        ...
        if record:
            ...
            post_pi_endpoint = f"{self.endpoint}('Invoice','DTD000135')"
            self.logger.info("ATTEMPTING TO GET INVOICE USING MULTIPLE PK")
            purchase_order = self.request_api(
                "GET", endpoint=post_pi_endpoint, params=self.params
            )

            # self.logger.info(f"purchase_invoice created succesfully with No {purchase_order_no}")
            return "DTD000135", True, state_updates
